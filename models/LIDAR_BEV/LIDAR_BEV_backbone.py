"""Code from PIXOR (https://github.com/philip-huang/PIXOR)
Modified by Mathis Finckh to extract feature from ZOD
"""
...
class LidarBEVBackbone(nn.Module):
    def __init__(self, lidar_channels):
        super().__init__()
        # reuse PIXOR backbone but change input channels
        self.backbone = BackBone(Bottleneck, [3, 6, 6, 3], use_bn=True)
        # overwrite first conv to match BEV channels : Z_SIZE = int((z_max - z_min) / grid_res)
        self.backbone.conv1 = conv3x3(lidar_channels, 32)

    def forward(self, bev):
        # bev: (B, C_bev, H, W)
        feat = self.backbone(bev)  # (B, 96, H', W')
        return feat
